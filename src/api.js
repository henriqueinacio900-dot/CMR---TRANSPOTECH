...
export async function listarContratosManutencao() {
  const { data, error } = await supabase
    ...
    .select('*, analista_consultor:consultores!contratos_manutencao_analista_consultor_id_fkey(nome)')
...
export async function marcarContratoFaturado(id, faturado) {
  return atualizarContratoManutencao(id, { faturado, faturado_em: faturado ? new Date().toISOString() : null })
}
