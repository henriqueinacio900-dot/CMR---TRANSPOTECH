...
export async function listarNegocios() {
  const { data, error } = await supabase
    .from('negocios')
    .select(`
      id, etapa, valor_cotacao, valor_final, temperatura, ultima_interacao_em,
...
export async function registrarInteracao({ negocio_id, tipo, atendeu, respondeu, faz_sentido_seguir, observacao }) {
  const consultor = await getMeuConsultor()
  const { data, error } = await supabase
    .from('interacoes')
    .insert({ negocio_id, tipo, atendeu, respondeu, faz_sentido_seguir, observacao, criado_por: consultor?.id })
    .select()
    .single()
  if (error) throw error
  return data
